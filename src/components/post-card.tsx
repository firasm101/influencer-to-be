...
import { Heart, MessageCircle, Share2, Eye, Instagram, Linkedin, Sparkles } from "lucide-react";
  ...
  return (
    <Card className="overflow-hidden">
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            {creatorHandle && <span className="text-sm font-medium">@{creatorHandle}</span>}
      ...
      <CardContent className="space-y-4">
        {/* Caption */}
        {post.caption && <p className="text-sm leading-relaxed line-clamp-3">{post.caption}</p>}
        ...
        {post.analysis && (
          <div className="space-y-3 rounded-lg border bg-muted/30 p-3">
            ...
            {post.analysis.whyItWorked && (
              <p className="text-sm text-muted-foreground">{post.analysis.whyItWorked}</p>
            )}
